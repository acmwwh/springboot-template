...
import com.example.acmwwh.common.util.CollectionUtils;
...
public abstract class AbstractPipelineFactory<T extends BasePipelineContext> {
    ...
    public T execute(T context, String bizPipelineType) {
        ...
        // 函数增强
        List<Stage<T>> enhancedFunctionList =
            executeChains.stream().map(this::aroundAspectFunc).collect(Collectors.toList());
        // 获取执行结果
        return getPipeResult(context, enhancedFunctionList).orElse(context);
    }
     ...
     *            入参，类型I
     * @param functionList
     *            pipeLine方法列表，每个函数的输入类型为I，输出类型为O
     * @return 执行结果，类型O
     */
    private Optional<T> getPipeResult(T context, List<Stage<T>> functionList) {
    ...
    protected boolean preContextCheck(String methodName, T context) {
        ...
        if (context.getResult() != null && !context.getResult().isSuccess()) {
            log.error("UserRightsPipelineFactory.execute [{}] error, pre method is failed with resultEnum:{}",
                methodName, context.getResult());
            return false;
        }
